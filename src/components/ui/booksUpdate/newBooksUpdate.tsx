            ...
            <div className="grid grid-cols-5 items-center gap-4 mx-2">
                <Link to={`/novels/${book.path}`} className="col-span-2 md:col-span-1">
                    <img
                        src={book.img}
                        alt={book.name + "Cover"}
                        className="min-w-24 h-32 object-cover object-center bg-center rounded-md cursor-pointer"
                    />
                </Link>
                <div className="info col-span-3 md:col-span-2">
                    <div className="flex flex-col justify-stretch gap-2">
                        <div className="title">
                            <p className="md:text-xl cursor-pointer hover:text-orange-500 transition-all duration-300 ease-in-out">
                                {book.name}
                            </p>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-1 md:hidden">
                            {book.categories.slice(0, 2).map((category, index) => (
                                <Link
                                    to={`/novels/?genres=${category.path}`}
                                    key={index}
